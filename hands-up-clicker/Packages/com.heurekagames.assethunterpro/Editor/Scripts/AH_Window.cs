...
namespace HeurekaGames.AssetHunterPRO
{
    public class AH_Window : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            {
                Heureka_WindowStyler.DrawCenteredImage(m_window, AH_EditorData.Icons.Achievement, "Project completely clean. Nothing to show here");
                return;
            }
        ...
        private bool checkEmptyFolder(string dataPath, List<string> emptyfolders)
        {
            ...
            {
                if (!files[i].EndsWith(".meta", StringComparison.InvariantCultureIgnoreCase))
                {
                    hasNonMetaFile = true;
                    break;
                }
            ...
            return (!hasNonMetaFile && !hasFolderWithContents);
        }

        //OLD CHECK. The code saw folders with only non-unity assets as empty, which deleted things it´shouldnt
        /*private bool checkEmptyFolder(string dataPath, List<string> emptyfolders)
        {
            if (dataPath.EndsWith(".git", StringComparison.InvariantCultureIgnoreCase))
                return false;
